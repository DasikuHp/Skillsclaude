...
1. **Clave de la ruta de Blender (EditorSettings).** El escéptico afirmó `filesystem/import/blender/blender3_path` (4.2+) apuntando a la **carpeta**. **Adjudicación (corregida tras verificación):** el PR [#85448](https://github.com/godotengine/godot/pull/85448) (mergeado 2024, Godot 4.3+) renombró la clave a `filesystem/import/blender/blender_path` (UI *Editor Settings → Filesystem → Import → Blender → Blender Path*) y la ruta apunta ahora al **ejecutable** de Blender (`/usr/bin/blender`, `C:/Program Files/Blender Foundation/Blender 4.x/blender.exe`), no a la carpeta. La etiqueta/carpeta "Blender 3 Path" es de 4.2 y NO aplica en 4.6. Crucial y poco documentado: son **dos settings** (Project Settings = enabled; Editor Settings = ruta al ejecutable). Esto es anti-stuck de primer orden.
